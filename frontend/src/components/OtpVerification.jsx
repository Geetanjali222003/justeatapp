...
const OtpVerification = () => {
  ...
  const email = useMemo(() => pending?.email || emailFromUrl, [pending, emailFromUrl]);
  ...
  const verifyOtp = async (e) => {
    ...
    if (!pending) {
      setToast({ show: true, message: "Registration session expired. Start again.", type: "error" });
      return;
    }

    if (!/^\d{4,6}$/.test(otp.trim())) {
      setFieldError("Enter a valid OTP");
      return;
    }
    ...
    try {
       ...
       */
      await registerApi({ ...pending, otp: otp.trim() });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8f8f8", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      ...
      <div style={{ background: "#fff", width: "100%", maxWidth: 430, borderRadius: 14, boxShadow: "0 8px 24px rgba(0,0,0,.08)", padding: "2rem" }}>
        <h3 style={{ margin: 0, color: "#fc8019", fontWeight: 800 }}>OTP Verification</h3>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, fontSize: ".85rem" }}>
          <button onClick={resendOtp} disabled={resending} style={{ border: "none", background: "transparent", color: "#fc8019", fontWeight: 700 }}>
            {resending ? "Resending..." : "Resend OTP"}
          </button>
          <Link to="/register" style={{ color: "#fc8019", textDecoration: "none", fontWeight: 700 }}>
            Back
          </Link>
        </div>
      </div>
